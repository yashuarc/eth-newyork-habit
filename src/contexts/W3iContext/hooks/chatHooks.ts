...
export const useChatState = (w3iProxy: Web3InboxProxy, proxyReady: boolean) => {
  ...
  const handleRegistration = useCallback(
    async (key: string) => {
      if (chatClient && key && uiEnabled.chat) {
        try {
          const registeredKeyRes = await chatClient.register({
  ...
  useEffect(() => {
    ...
    const inviteSentSub = chatClient.observe("chat_invite_sent", {
      next: refreshChatState,
    });
    ...
    const chatMessageSentSub = chatClient.observe("chat_message_sent", {
      next: refreshChatState,
    });
    const chatJoinedSub = chatClient.observe("chat_joined", {
      next: refreshChatState,
    });
    const inviteAcceptedSub = chatClient.observe("chat_invite_accepted", {
      next: refreshChatState,
    });
    const inviteRejectedSub = chatClient.observe("chat_invite_rejected", {
      next: refreshChatState,
    });
