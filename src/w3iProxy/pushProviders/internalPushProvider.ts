...
export default class InternalPushProvider implements W3iPushProvider {
  ...
  public initState(pushClient: NotifyClient) {
    ...
    this.pushClient.on("notify_message", (args) =>
      this.emitter.emit("notify_message", args)
    );
    this.pushClient.on("notify_subscriptions_changed", (args) =>
      this.emitter.emit("notify_subscriptions_changed", args)
    );
    this.pushClient.on("notify_update", (args) =>
      this.emitter.emit("notify_update", args)
    );
    this.pushClient.on("notify_delete", (args) =>
      this.emitter.emit("notify_delete", args)
    );
  ...
  public handleMessage(request: JsonRpcRequest<unknown>) {
    switch (request.method) {
      ...
      default:
        throw new Error(
          `Method ${request.method} unsupported by provider ${this.providerName}`
        );
    }
  }
  ...
  public async getMessageHistory(params: { topic: string }) {
    ...
    console.log(
      "InternalPushProvider > PushClient.getMessageHistory > messages",
      messages
    );
