...
export const getMessageId = (params: ChatClientTypes.Message) => {
  return hashMessage(
    `${params.message}${params.timestamp}${params.authorAccount}`
  );
};
...
class W3iChatFacade implements W3iChat {
  ...
  private handleMessagePublishing() {
    this.messageReplaySubject
      .pipe(filter((m) => m.status === "pending"))
      .subscribe((replayMessage) => {
        from(this.provider.message(replayMessage))
          .pipe(
            timeout({
              each:
                this.messageSendTimeout +
                replayMessage.count * this.messageSendTimeout,
  ...
  private subscribeToUnsentMessages() {
    this.messageReplaySubject
      .pipe(
        scan<ReplayMessage, Map<string, ReplayMessage>>(
          (messageMap, message) => {
            const isNewMessage = !messageMap.has(message.id);
            const isNewerMessage =
              messageMap.has(message.id) &&
              messageMap.get(message.id)?.status !== message.status;
